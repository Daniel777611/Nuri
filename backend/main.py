...
async def upsert_push_device(body: PushDeviceUpsert, uid: str = Depends(_req_uid)):
    ...
    # The dispatcher computes quiet hours from notification_preferences, whose
    # time_zone defaults to UTC. Storing the zone only on the device left every
    # parent on UTC: an American afternoon fell inside 21:00–08:00 and the
    # notification was held until night. The phone knows its zone, so the
    # registration that already carries it is what keeps the preference right —
    # merging only this column, so a parent's own quiet hours and caps survive.
    zone = (body.time_zone or "").strip()
    if zone:
        try:
            ZoneInfo(zone)
    ...
    if zone:
        def _sync_zone() -> None:
            sb.table("notification_preferences").upsert(
                {"user_id": uid, "time_zone": zone, "updated_at": now_iso},
                on_conflict="user_id",
            ).execute()
        try:
            await anyio.to_thread.run_sync(_sync_zone)
        except Exception as exc:  # noqa: BLE001 - the device is stored; the zone can follow next launch
            logging.getLogger("nuri.push").warning(
                "time zone sync failed: %s", type(exc).__name__,
            )
