...
## 核心功能
0. **账号注册（4 步向导）+ 登录**：邮箱+密码（bcrypt 哈希）+ JWT；4 个引导问题（昵称/身份、城市、最关心的育儿类别多选）；登录/注册成功后按"是否有孩子"路由到 onboarding 或 tabs；profile 页登出与"清空所有数据"都会清 token 并跳回登录/注册
1. **首页信息流**：三类卡片，底部 ★/↗/⟳ icon 行，点击卡片主体跳详情页
