...
### 步骤 4：任务卡二选一（**需要产品决定，代码不能替你选**）

- 他们：`task_proposals` 在主回复同一次调用里，零额外调用，卡片和正文保证一致
- 我们：router 判断 + `_plan_task_cards` 每话题每日额度，有 `route_reason` 可审计

建议组合而不是二选一：**用他们的生成方式，套我们的额度层**。他们那版没有解决
「一天出太多次」，而那正是测试反馈第 5 项的另一面。
